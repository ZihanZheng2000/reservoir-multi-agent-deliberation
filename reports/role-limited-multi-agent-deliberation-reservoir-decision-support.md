...
The research design tested here is that role-limited multi-agent deliberation may better approximate the epistemic structure of an actual approval process. Rather than asking one model to be all experts simultaneously, the system assigns bounded roles to expert agents and stakeholder agents, forces each to cite evidence, registers conflicts and minority positions, and asks a decision authority to produce a final recommendation from the complete deliberation record. The design has an expert-specialization intuition similar in spirit to mixture-of-experts systems, but it is not a neural MoE architecture: synthesis occurs through inspectable deliberation rather than hidden routing, learned gating, or a single aggregate prediction. This distinction matters because the paper's central contribution is visible role-bounded disagreement, not merely expert specialization. The design also draws on deliberative governance frameworks that emphasize the procedural and legitimacy value of structured multi-party reasoning in contested decisions [CITE].
...
The paper's argument can be summarized in five parts. The **problem** is that reservoir approval decisions require role-specific expert judgment and stakeholder conflict representation that are difficult to express through a single objective function or aggregate score. The **method** is a role-limited multi-agent deliberation workflow that separates expert and stakeholder perspectives, requires scenario-grounded evidence citation, and preserves disagreement before decision synthesis. The **benchmark** is an eight-case synthetic reservoir approval suite designed to test clear approvals, clear rejections, conditional approvals, redesign cases, emergency sequencing, transboundary governance, and near-tipping-point decisions. The **result** is that the multi-agent system produces stable decision directions across two model environments and shows the largest advantage over baselines in cases requiring sequencing, governance conditions, or conditional approval logic. The **contribution** is an inspectable decision-support framework and evaluation protocol for contested infrastructure decisions, not a claim that LLM agents can determine objectively correct reservoir outcomes.
